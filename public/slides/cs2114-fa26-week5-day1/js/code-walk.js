...
(function () {
  ...
  // or, when several lines share the step, covers them as one block (W4D1 04).
  function updatePC(sec, animated) {
    sec.querySelectorAll('.editor').forEach(function (ed) {
      var bar = ed.querySelector('.editor__pc');
      if (!bar) return;
      var curs = ed.querySelectorAll('.walkline.is-current');   // several lines on one step = one block
      if (!animated) bar.style.transition = 'none';
      if (curs.length) {
        var first = curs[0], last = curs[curs.length - 1];
