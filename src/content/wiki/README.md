...
### Quantitative-module components

Data for the walkthroughs lives in `src/data/quant-scenarios.ts`; GenAI toolbox
content in `src/data/genai-toolbox.ts`; the running-example CSVs are generated by
`scripts/generate-quant-data.mjs` into `public/templates/`.

| Component | Use it for | Example lesson |
| --- | --- | --- |
| `ScenarioWalkthrough` | scenario-based "which analysis and why" trainer; every option gives feedback | `quantitative/anova-and-categorical.mdx` |
| `GenAiToolbox` | per-lesson LLM best-practice aside (prompt + verify checklist + failure mode) | `quantitative/logistic-regression.mdx` |
| `GenAiGroundRules` | the module-wide GenAI rules box (rendered once) | `quantitative/overview.mdx` |
| `CodeTabs` | tabbed R / Python code panels | `quantitative/comparing-two-groups.mdx` |
| `IccExplorer` | live item characteristic curve (props: `showInfo`, `compare`) | `quantitative/irt-fundamentals.mdx` |
| `LogitTranslator` | odds ratio → probability / percentage-point translator | `quantitative/logistic-regression.mdx` |
| `ItemAnalysisTable` | sortable classical item stats from the DCI pilot | `quantitative/reliability-and-item-analysis.mdx` |
| `EffectSizeExplorer` | Cohen's d slider with overlap/CLES readout | `quantitative/effect-sizes-and-reporting.mdx` |
